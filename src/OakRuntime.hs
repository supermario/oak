...
run :: IO ()
run = Hilt.manage $ Hilt.program $ do
  let app = App.main
  ...
  mapM_
    ( \s -> case s of
      SubKeypress onKeypress -> serviceOrCreate c Console $ do
...
runCmd c cmd = case cmd of
  CmdSocketSend socketId text -> withSocketHandle c (\h -> SocketServer.send h socketId text)

  CmdSocketBroadcast text     -> withSocketHandle c (`SocketServer.broadcast`text)
